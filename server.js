...
function salvarHistorico(h) { try { fs.writeFileSync(HISTORICO_FILE, JSON.stringify(h, null, 2)); } catch(e) { console.error('Erro ao salvar histórico:', e.message); } }
  ...
  for (const [ambiente, items] of Object.entries(byAmbiente)) {
    ...
    for (const item of items) {
      for (const foto of item.fotos) {
        ...
        registrosParagraphs.push(new Paragraph({ children: [N(`Imagem ${imgCounter} - ${desc}`, 20)], indent, spacing: { after: 80 } }));
        try {
    ...
    sections: [
      {
        ...
        children: [new Paragraph({ children: [new ImageRun({ data: coverImgData, transformation: { width: 794, height: 1123 }, type: 'png' })], alignment: AlignmentType.CENTER, spacing: { before: 0, after: 0 } })]
      },
      {
...
// HTTP SERVER com limite de 50MB
  ...
  if (req.method === 'POST' && req.url === '/gerar-laudo') {
    ...
    req.on('data', chunk => {
      ...
      if (bodySize > MAX_SIZE) {
        ...
        res.end(JSON.stringify({ error: 'Payload muito grande. Reduza o número ou tamanho das fotos.' }));
        req.destroy();
        return;
      }
      body += chunk;
    ...
    req.on('end', async () => {
      try {
        ...
        console.log(`Gerando laudo para: ${payload.dados?.nome} - ${payload.registros?.length} registros`);
        ...
        console.log(`Laudo gerado: ${nomeArquivo} (${docxBuffer.length} bytes)`);
        ...
        console.error('Erro ao gerar laudo:', e.message, e.stack);
